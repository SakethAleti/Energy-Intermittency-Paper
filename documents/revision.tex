...
%	\date{\vspace{-1em}}
	...
	Furthermore, assuming a short time scale makes it simple to differentiate intermittent technologies. In this setting, we can use ``intermittent'' to refer to a technology where  output per unit, $\xi_{i,t}$, varies over time (with $t$).  This is because we define intermittency as variation in electricity output over time due to physical constraints. Based on our assumptions, the only source of variation in output over time comes from $\boldsymbol{\xi}$. Hence, we only need to consider $\boldsymbol{\xi}$ to determine which inputs are intermittent. 
